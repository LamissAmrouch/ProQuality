                        ...
                        <div class="page-header">
                            <div class="page-title">
                                <ol class="breadcrumb text-right">
                                    ...
                                    <li class="active"><a href="{{ route('alertRF.list')}}">Alertes retour fournisseur</a></li>
                                </ol>
                            </div>
                        </div>
                    </div>
                ...
                <div class="main-content">
                       <div class="card alert">
                                <div class="card-header">
                                    <div class="col-lg-6">
                                        <h4>Liste des alertes retours fournisseurs</h4>
                                    </div>
                                    <div class="col-lg-6">
                                        <div class="text-right"> 
                                            @role('simple') 
                                            <a href="{{ route('alertRF.create')}}">
                                                <button type="button" class="btn btn-primary btn-addon m-b-10 m-l-5">
                                                    <i class="ti-plus"></i>Ajouter un retour fournisseur</button>                 
                                            </a>
                                            @endrole
                                        </div>        
                                    </div>
                                    ...
                                    <table class="table table-responsive table-hover alert-table">
                                        ...
                                        @foreach($alerts as $alert)
                                            <tr @role('gestionnaire') 
                                                ...
                                                @role('simple')
                                                <td>
                                                    <a style="text-decoration:none;color:#ffffff;" href="{{ route('alertRF.view',$alert )}}"> 
                                                        <button class="btn btn-info btn-sm"> 
                                                            <i class="ti-eye" aria-hidden="true"></i> 
                                                        </button>
                                                    </a>
                                                    @if($alert->etat == "nouveau")
                                                    <a style="text-decoration:none;color:#ffffff;" href="{{ route('alertRF.edit',$alert )}}"> 
                                                        <button class="btn btn-warning btn-sm"> 
                                                            <i class="ti-pencil-alt" aria-hidden="true"></i> 
                                                        </button>
                                                    </a>                                                   
                                                    <form method="POST" id="delete-form-{{$alert->id}}" action="{{ route('alertRF.delete',$alert) }}" style="display:none;">
                                                        {{ csrf_field() }}
                                                        {{ method_field('delete')}}
                                                    </form>
